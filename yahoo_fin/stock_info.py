...
def get_data(ticker, start_date=None, end_date=None, index_as_date=True,
    ...
    if interval not in ("1d", "1wk", "1mo", "1m"):
        raise AssertionError(
            "interval must be of of '1d', '1wk', '1mo', or '1m'")
...
def tickers_niftybank():
    ...
    niftybank = ['AXISBANK', 'KOTAKBANK', 'HDFCBANK', 'SBIN', 'BANKBARODA', 'INDUSINDBK',
                 'PNB', 'IDFCFIRSTB', 'ICICIBANK', 'RBLBANK', 'FEDERALBNK', 'BANDHANBNK']
...
def tickers_ftse100(include_company_data=False):
    ...
    table = pd.read_html(
        "https://en.wikipedia.org/wiki/FTSE_100_Index", attrs={"id": "constituents"})[0]
...
def tickers_ftse250(include_company_data=False):
    ...
    table = pd.read_html(
        "https://en.wikipedia.org/wiki/FTSE_250_Index", attrs={"id": "constituents"})[0]
...
def get_stats_valuation(ticker):
    ...
    tables = [
        table for table in tables if "Trailing P/E" in table.iloc[:, 0].tolist()]
...
def get_earnings_in_date_range(start_date, end_date):
    ...
    dates = [current_date +
             datetime.timedelta(diff) for diff in range(days_diff + 1)]
